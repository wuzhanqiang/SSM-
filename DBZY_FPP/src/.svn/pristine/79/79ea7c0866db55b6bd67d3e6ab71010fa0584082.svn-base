...
import com.actionsoft.bpms.bpmn.modeler.model.node.task.imp.UserTask;
import com.actionsoft.bpms.server.UserContext;
import com.actionsoft.bpms.util.DBSql;
import com.actionsoft.sdk.local.SDK;
import com.nepharm5.apps.fpp.nepg.msg.WjpsMsg;

public class GczbSpsmRtclass extends ExecuteListener implements
	...
	public void execute(ProcessExecutionContext ctx) throws Exception {
		...
		if(ht!=null && !ht.isNew()){
			...
			double je = DBSql.getDouble("select sum(htjg) as sm from BO_DY_XZBG_GCZB_S where bindid ='"+bindid+"'", "sm");
			/*
			...
			*/
			...
			if(!spsm.equals("")){
				String sql = "update BO_DY_XZBG_GCZB_P set spsm = '"+spsm+"' where id = '"+id+"'";
				conn = DBSql.open();
				try {
		...
		//短信内容
		String msgContext = createUser+"发起的标题为【"+bt+"】的工程招标流程已流转至您的BPM系统，紧急程度为"
		                    +jjcd+",请于4小时内办理完毕！";
