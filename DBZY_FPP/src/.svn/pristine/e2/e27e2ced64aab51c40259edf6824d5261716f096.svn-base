...
public class WzzbSpsmRtclass extends ExecuteListener implements
	...
	public void execute(ProcessExecutionContext ctx) throws Exception {
		...
		if(ht!=null && !ht.isNew()){
			...
			double je = DBSql.getDouble("select sum(htjg) as sm from BO_DY_XZBG_WZZB_S where bindid ='"+bindid+"'", "sm");
			/*
			...
			*/
			...
			if(!spsm.equals("")){
				String sql = "update BO_DY_XZBG_WZZB_P set spsm = '"+spsm+"' where id = '"+id+"'";
				conn = DBSql.open();
				try {
