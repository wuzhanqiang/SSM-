...
public class LczcdrbhjyFORM_AFTER_SAVE extends ExecuteListener {
	...
	public void execute(ProcessExecutionContext arg0) throws Exception {
        ...
        List<BO> list = SDK.getBOAPI().query("BO_DY_ZDZC_LCZD_S").bindId(bindid).list();
        if(list.size() > 0) {
        	for(int i = 0; i < list.size(); i++) {
        		BO bo = list.get(i);
        		String ZDBH = bo.getString("ZDBH1");
    			String sqlCheck = "select COUNT(*) from BO_DY_ZDZC_LCZD_S WHERE ZDBH1 = '" + ZDBH + "'";
    			...
    			if(flag != 1) {
    				String a = SDK.getRuleAPI().executeAtScript("Lczd@sequenceYear(CRM_CUSTOMER,8,0)");
    				bo.set("ZDBH1",a);
