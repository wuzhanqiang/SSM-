...
public class RwgzfpTaskBeforeCompleteStep2 extends InterruptListener implements
		InterruptListenerInterface {

	public RwgzfpTaskBeforeCompleteStep2() {
		...
		if(f2){
			...
			for(int i=0;i<list.size();i++){
				...
				if("1".equals(JXFP) && uid.equals(JSRZH)){
			        ...
			        //boData.set("PBINDID", MainData.get("BINDID"));
			       
			        			        
			        SDK.getBOAPI().update("BO_DY_RWGL_RWGZFP_MAIN", boData);
				
			        //将附件更新到新分配的任务中
			        String targetBoId = SDK.getBOAPI().getByProcess("BO_DY_RWGL_RWGZFP_MAIN", SubBindid).getId();
			        PerformanceDao dao = new PerformanceDao();
					String targetTaskInstId=dao.getwfcTaskId(SubBindid);
			        //主表附件
			        SDK.getBOAPI().copyFileTo(MainData.getId(), "FJ", targetBoId, "BO_DY_RWGL_RWGZFP_MAIN", "FJ", SubBindid, targetTaskInstId);
			        //子表附件
			        SDK.getBOAPI().copyFileTo(list.get(i).getString("ID"), "FJ", targetBoId, "BO_DY_RWGL_RWGZFP_MAIN", "FJ", SubBindid, targetTaskInstId);
