...
import com.nepharm.apps.fpp2.biz.yyb.constant.ProductPlanConstant;

/**
 * 确定生产计划 生产经理 打开表单后，进行数据过滤
 * @author lizhen
 *
 */
public class ProductFormGridFilter extends FormGridFilterListener {
...
//		String userCompanyId="103";
		String userCompanyId=SDK.getRuleAPI().executeAtScript("@getUserInfo("+context.getUserContext().getUID()+",GSBM)");
//		String userCompanyName=SDK.getRuleAPI().executeAtScript("@getUserInfo(@uid,GSMC)");
		//当前表单子表名
		...
		switch (tableName) {
		//确定生产计划子表
		case ProductPlanConstant.TAB_SCJH_YLYSCJH_S:
