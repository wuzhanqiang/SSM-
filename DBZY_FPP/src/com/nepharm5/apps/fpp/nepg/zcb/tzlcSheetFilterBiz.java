...
public class tzlcSheetFilterBiz extends FormGridFilterListener implements
ValueListenerInterface{
	...
	public FormGridRowLookAndFeel acceptRowData(ProcessExecutionContext context, List<BOItemModel> boItemList, BO boData) {
		if (context.getParameterOfString(ListenerConst.FORM_EVENT_PARAM_BONAME).toString().equals("BO_DY_ZCB_TZLC_S")) {
			StringBuffer productManagers = new StringBuffer();
			String bindid = context.getProcessInstance().getId();
			List<BO> formData = SDK.getBOAPI().query("BO_DY_ZCB_TZLC_S").bindId(bindid).list();//.getByProcess("BO_DY_YSYJH_S", bindid);
			int i = 0;
