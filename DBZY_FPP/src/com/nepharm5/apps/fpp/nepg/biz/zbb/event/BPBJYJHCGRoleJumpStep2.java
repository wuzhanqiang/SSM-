...
public class BPBJYJHCGRoleJumpStep2 implements HumanPerformerInterface {

	public BPBJYJHCGRoleJumpStep2() {
	...
	public String getHumanPerformer(UserContext ctx, ProcessInstance ProcessInstance,
		...
		List<BO> boDatas = SDK.getBOAPI().query("BO_DY_DY_ZBB_BPBJYJHCG_S").bindId(ProcessInstance.getId()).list(); 
		
		String uid = "";
		for(BO bo:boDatas){
			uid += bo.getString("SQRZH")+" ";
		}
		return uid;
	}
