...
public class LczcbhwyFORM_AFTER_SAVE extends ExecuteListener{
	...
	public void execute(ProcessExecutionContext arg0) throws Exception {
        ...
        List<BO> list = SDK.getBOAPI().query("BO_DY_ZDZC_LCZC_S").bindId(bindid).list();
        if(list.size() > 0) {
        	for(int i = 0; i < list.size(); i++) {
    			...
    			if(flag != 1) {
    				String a = SDK.getRuleAPI().executeAtScript("Lczc@sequenceYear(CRM_CUSTOMER,8,0)");
    				bo.set("ZCBH1",a);
    	...
    	//BO操作
  		...
  		//获取子表集合
  		List<BO> datas = boapi.query("BO_DY_ZDZC_LCZC_S").addQuery("BINDID = ", bindid).list();
